...
class CalibrationWizard(ctk.CTkToplevel):
    ...
    def _set_collecting(self, kind: str):
        if not self._running or not self._status_lbl_alive():
            return
    ...
    def _status_lbl_alive(self) -> bool:
        try:
            return bool(self.status_lbl.winfo_exists())
        except Exception:
            return False

    def _update_status(self):
        # Step-2 widget may be gone if both classes finished and the wizard
        # transitioned to the training step before a key-release fired.
        if not self._running or not self._status_lbl_alive():
            return
