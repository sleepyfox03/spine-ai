...
class SessionTracker:
    ...
    def _reset(self):
        ...
        # Per-minute timeline for the Sitting Time tab — one entry per
        # completed minute, value is the majority state for that minute:
        # 'good' | 'bad' | 'away'.
        self._minutes: list[str]   = []
        self._minute_buffer_count  = 0
        self._minute_counts        = {'good': 0, 'bad': 0, 'away': 0}
    ...
    def tick(self, is_active: bool, label: str, score: float):
        with self._lock:
            ...
            # Per-minute timeline accumulation
            if is_active:
                key = 'good' if label.lower() == 'good' else 'bad'
            else:
                key = 'away'
            self._minute_counts[key] += 1
            self._minute_buffer_count += 1
            if self._minute_buffer_count >= 60:
                majority = max(self._minute_counts, key=self._minute_counts.get)
                self._minutes.append(majority)
                self._minute_counts = {'good': 0, 'bad': 0, 'away': 0}
                self._minute_buffer_count = 0

    # ── Minute timeline ───────────────────────────────────────────────────────

    def get_minute_timeline(self) -> list[str]:
        """Snapshot copy of the per-minute state list ('good' | 'bad' | 'away')."""
        with self._lock:
