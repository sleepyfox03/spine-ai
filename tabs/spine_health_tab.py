...
class SpineHealthTab(ctk.CTkScrollableFrame):
    ...
    def _live_bio_age(self) -> int:
        """Read the saved profile age, falling back to USER_AGE."""
        try:
    ...
    def _live_spine_age(self) -> float:
        bio_age = self._live_bio_age()
        if self._app is not None and getattr(self._app, 'session_tracker', None):
            return self._app.session_tracker.spine_age(bio_age)
        return float(bio_age)
    ...
    def refresh(self):
        ...
        bio_age   = self._live_bio_age()
        self._bio_age = bio_age   # cache for the arc-redraw
