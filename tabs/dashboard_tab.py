...
class DashboardTab(ctk.CTkScrollableFrame):
    ...
    def _tick_recovery(self):
        ...
        if self._recovery_countdown <= 0:
            # Timer expired — user didn't correct posture in time
            self._recovery_active = False
            try:
                from components.notification_popup import NotificationPopup
                NotificationPopup(
                    title="Posture Alert",
                    message="You've been slouching too long. Stand up and stretch.",
                    alert_type="danger",
                )
            except Exception:
                pass
            self.recovery_heading.configure(
                text="Recovery Time Exceeded",
                text_color=ACCENT_RED,
            )
            self.recovery_countdown_lbl.configure(text="0")
            self.recovery_bar.set(0)
            self.recovery_sub.configure(
                text="Take a stretch break now",
                text_color=ACCENT_RED,
            )
            return
