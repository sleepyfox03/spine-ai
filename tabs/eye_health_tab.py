...
class EyeHealthTab(ctk.CTkScrollableFrame):
    ...
    def update_blink(self, bpm: float, dist_cm: float, strain: float | None = None):
        ...
        """
        self._last_bpm = bpm
        bpm_unavailable = bpm < 0   # sentinel: Face Mesh disabled / Python 3.13

        if strain is None:
            # Skip the BPM contribution to strain when blink rate is unavailable.
            bpm_term = 0 if bpm_unavailable else max(0, (15 - bpm) * 5)
            strain = max(0, min(100,
                round(bpm_term + max(0, (50 - dist_cm) * 1.2))
            ))

        if bpm_unavailable:
            # Render "—" and a one-line explanation; skip the rest of the
            # BPM-driven UI logic (color thresholds, stare toast, alert).
            self.lbl_blink_val.configure(text="—", text_color=TEXT_SECONDARY)
            self.lbl_blink_status.configure(
                text="Face landmarks unavailable",
                text_color=TEXT_SECONDARY,
            )
            self.bpm_bar.set(0)
            self.bpm_bar.configure(progress_color=BG_SECONDARY)
            self._hide_stare_toast()

            # Distance + strain still render normally below.
            self._render_distance(dist_cm)
            self._render_strain(strain)
            return
    ...
    def _render_distance(self, dist_cm: float):
        if dist_cm >= 70:
            d_label, dist_color = "TOO FAR",   "#ffaa00"
        elif dist_cm >= 55:
            d_label, dist_color = "IDEAL",     ACCENT_PRIMARY
        elif dist_cm >= 40:
            d_label, dist_color = "CLOSE",     "#ffaa00"
        else:
            d_label, dist_color = "TOO CLOSE", ACCENT_RED
    ...
    def _render_strain(self, strain: float):
    ...
    def tick_active(self, _is_active: bool):
        """
        Called once per second from app._poll. The actual elapsed seconds and
        popup firing live on SpineAIApp (so the timer survives tab reopen);
        this method only renders the countdown label.
        """
        elapsed = 0
        target  = 40 * 60
        if self._app is not None:
            elapsed = getattr(self._app, '_40_elapsed', 0)
            target  = getattr(self._app, '_40_target', 40 * 60)
